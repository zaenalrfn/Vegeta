...
const FilterRating: React.FC<FilterRatingProps> = ({ value = [], onChange }) => {
    ...
    <>
      ...
      <div className="flex flex-col gap-2 my-4">
        <div className="flex gap-2 items-center">
          ...
          />
          <label htmlFor={"all"} className="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex gap-2 cursor-pointer">
            Semua Bintang
          </label>
        </div>
          ...
          <div key={`rating_${index}`} className="flex gap-2 items-center">
            ...
            <label
              ...
              {Array(+rate)
                .fill(null)
                .map((_, index) => {
                  return <IconStar key={`stars_${rate}_${index}`} className="w-5 h-5" fill="carrot" stroke="carrot" />;
                })}
            </label>
          </div>
        ))}
